...
import { useDocuments, useDocumentUpload } from '@/lib/hooks'
...
export default function SummarizerPage() {
  ...
  // Use real API hooks
  const { data: summaries, isLoading, error, refetch } = useDocuments()
  ...
  const handleFile = async (file: File) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      toast.error('Please upload a PDF, DOC, DOCX, TXT, or MD file')
      return
    }
    ...
    try {
      ...
      if (result) {
        toast.success('Document uploaded and summarized successfully!')
        setActiveTab('history')
        refetch() // Refresh the documents list
      } else {
        toast.error(uploadError || 'Failed to process document')
      }
    } catch (error) {
